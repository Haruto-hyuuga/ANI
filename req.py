...
async def recommend_anime_button(anime_id: int) -> None:
    ...
    buttons.append([InlineKeyboardButton("Don't forget to give a 3 Stars 🌟 Review")])
...
from config import BOT_C_url, GROUP_url, Dub_C_url, Sub_C_url
...
"""
    try:
        ...
        button = (
            [
                InlineKeyboardButton("⛩️ 𝗝𝗮𝗽𝗮𝗻𝗲𝘀𝗲 𝗦𝗨𝗕 🌸", url=Sub_C_url),
                InlineKeyboardButton("🎐 𝗘𝗻𝗴𝗹𝗶𝘀𝗵 𝗗𝗨𝗕 🗺️", url=Dub_C_url)
            ],
            [
                InlineKeyboardButton("💠 𝗕𝗢𝗧 𝗨𝗣𝗗𝗔𝗧𝗘𝗦 💠", url=BOT_C_url),
                InlineKeyboardButton("💬 𝗔𝗡𝗜𝗠𝗘 𝗚𝗥𝗢𝗨𝗣 💬", url=GROUP_url)
            ]
        )
